...
class TestIgnoredObjectFields:
    ...
    def test_is_set_of_strings(self) -> None:
...
class TestPackagesMap:
    ...
    def test_is_dict(self) -> None:
        """PACKAGES_MAP should be a dict mapping import names to
        package names."""
        assert isinstance(PACKAGES_MAP, dict)

    def test_contains_known_mappings(self) -> None:
...
class TestGetModule:
    ...
    def test_single_module_import(self) -> None:
    ...
    def test_module_with_attribute_path(self) -> None:
    ...
    def test_nested_module_with_attribute(self) -> None:
...
class TestGetChild:
    ...
    def test_get_existing_attribute_from_module(self) -> None:
    ...
    def test_get_existing_attribute_from_object(self) -> None:
    ...
    def test_nonexistent_attribute_on_nonmodule(self) -> None:
    ...
    def test_nonexistent_attribute_on_empty_module(self) -> None:
        """Should raise ImportError suggesting circular import for
        empty module."""
        ...
        with (
            patch.object(importer, "ismodule", return_value=False),
    ...
    def test_nonexistent_attribute_on_module(self) -> None:
    ...
    def test_force_import_on_module(self) -> None:
...
class TestCachedImport:
    ...
    def test_returns_same_as_import_object(self) -> None:
...
class TestRequired:
    ...
    def test_import_existing_attribute(self) -> None:
    ...
    def test_import_nonexistent_with_throw_true(self) -> None:
        """Should raise ImportError for non-existent module."""
        with pytest.raises(ImportError) as exc_info:
            required("nonexistent_xyz_module")
        assert "couldn't import required" in str(exc_info.value)

    def test_import_nonexistent_with_throw_false(self) -> None:
    ...
    def test_falls_back_to_import_object_on_typeerror(self) -> None:
        """Should fallback to import_object on TypeError."""

        def side_effect(*args: object, **kw: object) -> Never:
            raise TypeError("test error")

        with (
            patch("kain.importer.cached_import", side_effect=side_effect),
            patch("kain.importer.import_object") as mock_import,
        ):
...
class TestOptional:
    ...
    def test_defaults_to_quiet_true(self) -> None:
    ...
    def test_defaults_to_throw_false(self) -> None:
    ...
    def test_returns_object_for_existing(self) -> None:
...
class TestGetPath:
    ...
    def test_get_path_with_explicit_root(self, tmp_path: Path) -> None:
    ...
    def test_get_path_single_dot(self, tmp_path: Path) -> None:
    ...
    def test_get_path_double_dot(self, tmp_path: Path) -> None:
        """Should handle double dot."""
        root = tmp_path / "test" / "dir"
        root.mkdir(parents=True, exist_ok=True)
        result = get_path("..", root=root)
        assert result.name == "test"

    def test_get_path_triple_dot(self, tmp_path: Path) -> None:
    ...
    def test_get_path_quad_dot(self, tmp_path: Path) -> None:
        """Should handle four dots."""
        root = tmp_path / "test" / "dir" / "subdir"
        root.mkdir(parents=True, exist_ok=True)
        result = get_path("....", root=root)
        assert result == tmp_path

    def test_get_path_startswith_dotdot_slash(self, tmp_path: Path) -> None:
    ...
    def test_get_path_finds_directory_by_name(self, tmp_path: Path) -> None:
    ...
    def test_get_path_not_found_raises(self, tmp_path: Path) -> None:
    ...
    def test_get_path_with_sep_in_path(self, tmp_path: Path) -> None:
...
class TestAddPath:
    ...
    def test_add_directory_to_sys_path(self, tmp_path: Path) -> None:
        """Should add directory to sys.path."""
        test_path = tmp_path / "test_add_path_xyz"
    ...
    def test_add_file_uses_parent_directory(self, tmp_path: Path) -> None:
        """Should use parent directory when path is a file."""
        test_dir = tmp_path / "test_add_path_dir"
        test_file = test_dir / "file.py"
        test_dir.mkdir(parents=True, exist_ok=True)
        test_file.touch()

        str_dir = str(test_dir.resolve())
        # Ensure paths don't exist initially
        for p in [str_dir, str(test_file), str(test_dir.resolve())]:
            if p in sys.path:
                sys.path.remove(p)
    ...
    def test_does_not_duplicate_existing_path(self, tmp_path: Path) -> None:
        """Should not duplicate existing paths in sys.path."""
        test_path = tmp_path / "test_add_path_dup"
        test_path.mkdir(parents=True, exist_ok=True)
    ...
    def test_add_path_logs_info(self, tmp_path: Path) -> None:
        """Should log info about added path."""
        test_path = tmp_path / "test_add_path_log"
        test_path.mkdir(parents=True, exist_ok=True)
        str_path = str(test_path.resolve())
    ...
    def test_add_path_raises_when_get_path_fails(self) -> None:
...
class TestEdgeCases:
    ...
    def test_import_object_with_deeply_nested_path(self) -> None:
    ...
    def test_required_with_extra_kwargs(self) -> None:
    ...
    def test_get_path_auto_root_detection(self) -> None:
    ...
    def test_get_path_many_dots(self) -> None:
    ...
    def test_import_object_circular_import_detection(self) -> None:
        ...
        with (
            patch("kain.importer.get_module") as mock_get_module,
            patch.object(importer, "ismodule", return_value=False),
        ):
...
class TestIntegration:
    ...
    def test_optional_fallback_chain(self) -> None:
    ...
    def test_path_resolution_and_add(self, tmp_path: Path) -> None:
        """Test path resolution and adding to sys.path."""
        root = tmp_path / "test_integration"
        subdir = root / "subdir"
        subdir.mkdir(parents=True, exist_ok=True)
        str_subdir = str(subdir.resolve())

        # Clean up any existing entries
        if str_subdir in sys.path:
            sys.path.remove(str_subdir)
...
class TestAddPathExtended:
    ...
    def test_add_path_with_absolute_string(self, tmp_path: Path) -> None:
        test_path = tmp_path / "test_add_path_str"
        test_path.mkdir(parents=True, exist_ok=True)
        str_path = str(test_path.resolve())
        if str_path in sys.path:
            sys.path.remove(str_path)
    ...
    def test_add_path_with_absolute_path_object(self, tmp_path: Path) -> None:
        test_path = tmp_path / "test_add_path_obj"
        test_path.mkdir(parents=True, exist_ok=True)
        str_path = str(test_path.resolve())
        if str_path in sys.path:
            sys.path.remove(str_path)
    ...
    def test_add_path_duplicate_handling(self, tmp_path: Path) -> None:
        test_path = tmp_path / "test_add_path_dup2"
        test_path.mkdir(parents=True, exist_ok=True)
        str_path = str(test_path.resolve())
        if str_path not in sys.path:
            sys.path.append(str_path)
    ...
    def test_add_path_with_root_kwarg(self) -> None:
        with patch("kain.importer.get_path") as mock_get_path:
            ...
            add_path("src", root="/custom/root")
            mock_get_path.assert_called_once()

    def test_add_path_forwards_kwargs_to_get_path(self) -> None:
        with patch("kain.importer.get_path") as mock_get_path:
            mock_get_path.return_value = Path("/fwd")
            add_path("src", root="/fwd", custom=True)
            assert mock_get_path.call_args[1]["custom"] is True

    def test_add_path_logs_info_when_added(self, tmp_path: Path) -> None:
        test_path = tmp_path / "test_add_path_log2"
        test_path.mkdir(parents=True, exist_ok=True)
        str_path = str(test_path.resolve())
        if str_path in sys.path:
            sys.path.remove(str_path)
    ...
    def test_add_path_no_log_when_duplicate(self, tmp_path: Path) -> None:
        test_path = tmp_path / "test_add_path_nolog"
        test_path.mkdir(parents=True, exist_ok=True)
        str_path = str(test_path.resolve())
        if str_path not in sys.path:
            sys.path.append(str_path)
    ...
    def test_add_path_file_uses_parent(self, tmp_path: Path) -> None:
        test_dir = tmp_path / "test_add_path_file_parent"
...
class TestGetChildExtended:
    ...
    def test_get_child_empty_module_suggests_circular_import(self) -> None:
        ...
        with (
            patch.object(importer, "ismodule", return_value=True),
...
class TestErrorCases:
    ...
    def test_permission_error_on_import_mocked(self) -> None:
        cached_import.cache_clear()
        with (
            patch("kain.importer.cached_import", side_effect=TypeError),
            patch(
            ...
            try:
                ...
                pytest.fail(
                    f"Expected PermissionError, got"
                    f" {type(e).__name__}: {e}",
                )
    ...
    def test_get_path_not_found_various(
        self,
        tmp_path: Path,
        name: str,
    ) -> None:
    ...
    def test_add_path_raises_on_unresolvable_relative(self) -> None:
        with (
            patch("kain.importer.get_path", return_value=None),
...
class TestIntegrationExtended:
    ...
    def test_required_optional_add_path_workflow(self, tmp_path: Path) -> None:
        import os
        ...
        test_dir = tmp_path / "test_integration_workflow"
        test_dir.mkdir(parents=True, exist_ok=True)
        str_dir = str(test_dir.resolve())
        if str_dir in sys.path:
            sys.path.remove(str_dir)
    ...
    def test_get_path_and_add_path_relative_resolution(
        self,
        tmp_path: Path,
    ) -> None:
        root = tmp_path / "test_rel"
        sub = root / "sub"
        sub.mkdir(parents=True, exist_ok=True)
        str_sub = str(sub.resolve())
        if str_sub in sys.path:
            sys.path.remove(str_sub)
        try:
