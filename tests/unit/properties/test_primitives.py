...
class TestInvocationContextCheck:
    """Paranoid edge-case coverage for ``invocation_context_check``."""

    def test_raises_when_klass_true_and_node_is_none(self) -> None:
        """
        Given: a descriptor with klass=True (class-only)
        When: invocation_context_check sees node=None
        Then: ContextFaultError is raised.
        """
        ...
        class Descriptor:
            ...
            def header_with_context(self, node: object) -> str:
                return f"header({node!r})"

        desc = Descriptor()
        wrapped = invocation_context_check(lambda _s, _n: 1)

        with pytest.raises(ContextFaultError):
            wrapped(desc, None)

    def test_raises_when_klass_true_and_node_is_instance(self) -> None:
        """
        Given: a descriptor with klass=True
        When: node is an instance (not a class)
        Then: ContextFaultError is raised.
        """
    ...
    def test_passes_when_klass_true_and_node_is_class(self) -> None:
        """
        Given: a descriptor with klass=True
        When: node is a class
        Then: the wrapped function is called.
        """
        ...
        desc = Descriptor()
        wrapped = invocation_context_check(lambda _s, n: n)

        assert wrapped(desc, type) is type

    def test_raises_when_klass_false_and_node_is_none(self) -> None:
        """
        Given: a descriptor with klass=False (instance-only)
        When: node is None
        Then: ContextFaultError is raised.
        """
    ...
    def test_raises_when_klass_false_and_node_is_class(self) -> None:
        """
        Given: a descriptor with klass=False
        When: node is a class
        Then: ContextFaultError is raised.
        """
    ...
    def test_passes_when_klass_false_and_node_is_instance(self) -> None:
        """
        Given: a descriptor with klass=False
        When: node is an instance
        Then: the wrapped function is called.
        """
        ...
        obj = object()
        assert wrapped(desc, obj) is obj

    @pytest.mark.xfail(
        strict=True,
        reason="invocation_context_check skips None check when klass=None",
    )
    def test_raises_when_klass_none_and_node_is_none(self) -> None:
        """
        Given: a descriptor with klass=None (mixed)
        When: node is None
        Then: ContextFaultError is raised.
        """
        ...
        with pytest.raises(ContextFaultError, match="non-instance invocation"):
            wrapped(desc, None)

    def test_passes_when_klass_none_and_node_is_instance(self) -> None:
        """
        Given: a descriptor with klass=None
        When: node is an instance
        Then: the wrapped function is called.
        """
    ...
    def test_passes_when_klass_none_and_node_is_class(self) -> None:
        """
        Given: a descriptor with klass=None
        When: node is a class
        Then: the wrapped function is called.
