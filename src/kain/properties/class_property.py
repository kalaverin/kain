...
class class_property[T_co](BaseProperty[T_co]):
    ...
    @override
    def title(self) -> str:
        return f"class descriptor {_who.Addr(self)}".strip()
...
class mixed_property[T_co](BaseProperty[T_co]):
    ...
    * ``Cls.attr``   → ``function(Cls)``
    ...
    @override
    def title(self) -> str:
        return f"mixed descriptor {_who.Addr(self)}".strip()
    ...
    def get_node(self, node: Any) -> Any:
        if node is None:
            msg = f"{self.header_with_context(node)}, node={node!r}"
            raise ContextFaultError(msg)
        return get_owner(node, self.name) if _is.Class(node) else node
