    ...
    * When called **with** ``stamp`` (during ``call`` / cache lookup), it
    ...
    def ttl(cls, expire: float) -> Callable[..., Any]:
        if not isinstance(expire, float | int):  # type: ignore[redundant-expr]
            msg = f"expire must be float or int, not {_who.Cast(expire)}"
            raise TypeError(msg)
        if expire <= 0:
            msg = f"expire must be a positive number, not {_who.Cast(expire)}"
...
class class_parent_cached_property[T_co](
    ...
    * With ``is_actual``    → ``cache[name] = (value, stamp)``
    ...
    def __init__(
        ...
        if method := getattr(_is.classOf(self), "is_actual", None):
            if is_actual:
                msg = (
                    f"{_who.Is(self)}.is_actual method ({_who.Cast(method)}) "
    ...
    @override
    def title(self) -> str:
        return f"class data-descriptor {_who.Addr(self)}".strip()
    ...
    def get_node(self, node: Any) -> Any:
        if node is None or not _is.Class(node):
