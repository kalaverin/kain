...
def proxy_to(  # noqa: PLR0915
    ...
    *mapping:
        ...
        The first element is the **pivot** — either:

        * a ``str`` naming an attribute on the instance (e.g. ``"inner"``), or
        * an arbitrary object used directly as the source.

        All remaining elements (except an optional trailing bind specifier)
        are the names of attributes to proxy.

        The last element may optionally control binding:

        * ``str`` → uses :class:`bound_property` (default behaviour).
        * ``None`` → direct copy mode; the attribute is copied verbatim from
          the class-level pivot descriptor.
        * Any other callable → treated as a custom descriptor factory.

    getter:
        Callable that receives an attribute name and returns a callable able
        to extract that attribute from the pivot entity. Defaults to
        :func:`operator.attrgetter`.

    default:
        Fallback value returned when the pivot is ``None`` or the proxied
        attribute is missing. Defaults to the sentinel ``Nothing``; in that
        case an appropriate exception is raised instead of returning a
        fallback.

    pre:
        Optional post-processing callable. When provided, the proxy result is
        wrapped with :func:`functools.partial(pre, result)`, so the attribute
        access returns a callable that must be invoked to obtain the final
        value.

    safe:
        If ``True`` (default), raises ``TypeError`` when attempting to proxy
        a public attribute (name not starting with ``_``) that already exists
        on the class. Set to ``False`` to allow overwriting existing
        attributes.
    ...
    ------
    TypeError
        If the decorated object is not a class, or if ``safe=True`` and a
        public attribute already exists on the class.
    ValueError
        If no attributes are requested for proxying.
    AttributeError
        If the pivot is missing, the pivot is ``None`` and no ``default`` is
        provided, or the proxied attribute does not exist on the pivot entity
    ...
    def class_wrapper(cls):  # noqa: PLR0915
        if not Is.Class(cls):
        ...
        for method in mapping_list:
            ...
            def wrapper(name, node):
                # When ``pivot`` is not a string, it is an object used directly
                # as the source, ignoring ``node`` entirely.
